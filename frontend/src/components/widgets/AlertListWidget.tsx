...
interface Props {
  data: AlertListData | AlertListData["alerts"]; // accepts both shapes
}
...
export const AlertListWidget: React.FC<Props> = ({ data }) => {
  // Handle both shapes:
  //   data_key: "alerts"         → data = { count, alerts: [...] }
  //   data_key: "alerts.alerts"  → data = [...]
  const alerts = Array.isArray(data) ? data : (data as AlertListData)?.alerts ?? [];

  if (!alerts.length) {
    return (
      <p style={{ fontFamily: "'Barlow', sans-serif", fontSize: 13, color: R.textMuted }}>
        No alerts found.
      </p>
    );
  }
